...
if __name__ == '__main__':
    ...
    positive_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(pred_positive)])
    negative_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(pred_negative)])
    neutral_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(pred_neutral)])
    ...
    n_top10_positive = min(200, n_pos) #(n_pos * args.positive)//100
    n_top10_negative = min(200, n_neg)#(n_neg * args.negative)//100
    ...
    positive_top10_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(sorted_pred_positive) if i < n_top10_positive ])
    negative_top10_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(sorted_pred_negative) if i < n_top10_negative ])
    neutral_top10_accuracy = np.array([pred_label == label for i, (tweet_id, sentence, pred_label, label, score) in enumerate(sorted_pred_neutral) if i < n_top10_neutral ])
    ...
    add_hindi = []
    transliterated = load_data('hindi_transliteration_binary_dict.pkl')
    for (tweet_id, sentence, pred_label, label, score) in top_confidence_positive:
        if tweet_id in transliterated:
            add_hindi.append((tweet_id, transliterated[tweet_id][0], pred_label, label, score))

    print('Before adding hindi:', len(top_confidence_positive), len(add_hindi))
    top_confidence_positive += add_hindi 
    print('Before adding hindi:', len(top_confidence_positive))
    ...
    chosen_sentences = [sentence for (tweet_id, sentence, pred_label, label, score) in fine_tune_data]
